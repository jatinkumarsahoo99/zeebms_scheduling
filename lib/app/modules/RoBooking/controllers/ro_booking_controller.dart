...
class RoBookingController extends GetxController {
  ...
  Map tabs = {
    "Deal": KeepAlive(keepAlive: true, child: DealView()),
    "Programs": KeepAlivePage(
      child: ProgramView(),
    ),
    "Spots": KeepAlivePage(
      child: SpotsView(),
    ),
    "Make Good Spots": KeepAlivePage(
      child: MakeGoodSpotsView(),
    ),
    "Booking Summary": KeepAlivePage(
      child: BookingSummaryView(),
    ),
    "Spots Not Verified": KeepAlivePage(
      child: SpotNotVerifiedView(),
    ),
    "Verify Spots": KeepAlivePage(
      child: VerifySpotsView(),
    ),
  };
  final count = 0.obs;
  @override
  ...
  getDisplay() {
    if (selectedLocation == null && selectedChannel == null) {
      LoadingDialog.callErrorMessage1(
          msg: "Location and channelis must to select.", callback: () {});
    } else {
      Get.find<ConnectorControl>().POSTMETHOD(
  ...
  pickFile() async {
    FilePickerResult? result = await FilePicker.platform
        .pickFiles(allowedExtensions: ["xlsx", "xlsm", "xls", "xlsb", "xltx"]);
