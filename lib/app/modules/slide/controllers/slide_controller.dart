...
class SlideController extends GetxController {
  ...
  var telecastedateTC = TextEditingController(),
      importDateTc = TextEditingController();
    ...
    Get.find<ConnectorControl>().GETMETHODCALL(
        ...
        fun: (resp) {
          ...
          if (resp != null &&
              resp is Map<String, dynamic> &&
              resp['pageload'] != null &&
              resp['pageload']['lstLocaction'] != null) {
            locationList.clear();
            locationList
                .addAll((resp['pageload']['lstLocaction'] as List<dynamic>)
                    .map((e) => DropDownValue(
        ...
        Get.find<ConnectorControl>().POSTMETHOD(
          ...
          fun: (resp) {
            ...
            if (resp != null &&
              ...
              dataTableList.clear();
              lastSelectedIdx = 0;
              dataTableList.value.addAll((resp['lstFPC'] as List<dynamic>)
          ...
          json: {
            ...
            "telecastDate": DateFormat("yyyy-MM-dd")
                .format(DateFormat("dd-MM-yyyy").parse(telecastedateTC.text)),
            "import": false,
            "importdate": DateFormat("yyyy-MM-dd")
                .format(DateFormat("dd-MM-yyyy").parse(importDateTc.text)),
          },
        );
      } catch (e) {
    ...
    Get.find<ConnectorControl>().POSTMETHOD(
      ...
      fun: (resp) {
        ...
        if (resp != null &&
            resp is Map<String, dynamic> &&
            resp['postsave'] != null &&
            resp['postsave'].toString().contains("Record")) {
  ...
  onEdit(PlutoGridOnChangedEvent event) {
    ...
    if (event.columnIdx == 3) {
      dataTableList.value[event.rowIdx].stationIdCheck =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 4) {
      dataTableList.value[event.rowIdx].presentsCheck =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 5) {
      dataTableList.value[event.rowIdx].presentationCheck =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 6) {
      dataTableList.value[event.rowIdx].commProgCheck =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 7) {
      dataTableList.value[event.rowIdx].commMenuCheck =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 8) {
      dataTableList.value[event.rowIdx].commUpTom =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 9) {
      dataTableList.value[event.rowIdx].networkId =
          (event.value.toString() == "true");
    } else if (event.columnIdx == 10) {
      dataTableList.value[event.rowIdx].marrideId =
          (event.value.toString() == "true");
    }
  }
}
